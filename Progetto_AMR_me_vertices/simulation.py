...
class Hrp4Controller(dart.gui.osg.RealTimeWorldNode):
    def __init__(self, world, hrp4):
        ...
        reference = [(0.07, 0., -0.1)] * 5 + [(0.05, 0., -0.0)] * 10 + [(0.05, 0., 0.)] * 10
        ...
        with open("Pos Lfoot pre trj", "w") as file:
            for i in range(len(self.pre_left_traj)):
                file.writelines(" ".join(map(str, self.pre_left_traj[i][0]['pos']))+ "\n")
            
        
        with open("Pos Rfoot pre trj", "w") as file:
            for i in range(len(self.pre_left_traj)):
                file.writelines(" ".join(map(str, self.pre_right_traj[i][0]['pos']))+ "\n")
