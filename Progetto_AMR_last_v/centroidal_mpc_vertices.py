  ...
  def __init__(self, initial, footstep_planner, params, CoM_ref, contact_trj_l, contact_trj_r):
    ...
    for i in range(self.N):
      ...
      for j in range(3):  # x, y, z
          self.opt.subject_to(self.opti_pos_contact_l[j, i] - self.opti_pos_contact_trj_l_ref[j, i] <= 0.02)
          self.opt.subject_to(self.opti_pos_contact_l[j, i] - self.opti_pos_contact_trj_l_ref[j, i] >= -0.02)

          self.opt.subject_to(self.opti_pos_contact_r[j, i] - self.opti_pos_contact_trj_r_ref[j, i] <= 0.02)
          self.opt.subject_to(self.opti_pos_contact_r[j, i] - self.opti_pos_contact_trj_r_ref[j, i] >= -0.02)

            #for orientation 0.02rad= 1.43° , 
          self.opt.subject_to(self.opti_ang_contact_l[j, i] - self.opti_ang_contact_trj_l_ref[j, i] <= 0.025)
          self.opt.subject_to(self.opti_ang_contact_l[j, i] - self.opti_ang_contact_trj_l_ref[j, i] >= -0.025)

          self.opt.subject_to(self.opti_ang_contact_r[j, i] - self.opti_ang_contact_trj_r_ref[j, i] <= 0.025)
          self.opt.subject_to(self.opti_ang_contact_r[j, i] - self.opti_ang_contact_trj_r_ref[j, i] >= -0.025)
    ...
    for i in range(self.N):
        ...
        cost += weight*100*cs.sumsqr(self.opti_hw[:,i+1]) + \
              ...
              cs.sumsqr((self.opti_pos_contact_l[:,i]-self.opti_pos_contact_l_ref[:,i])*self.opti_contact_left[i]) + \
              cs.sumsqr((self.opti_pos_contact_r[:,i]-self.opti_pos_contact_r_ref[:,i])*self.opti_contact_right[i]) + \
              cs.sumsqr((self.opti_ang_contact_l[:,i]-self.opti_ang_contact_l_ref[:,i])*self.opti_contact_left[i]) + \
              cs.sumsqr((self.opti_ang_contact_r[:,i]-self.opti_ang_contact_r_ref[:,i])*self.opti_contact_right[i]) + \
