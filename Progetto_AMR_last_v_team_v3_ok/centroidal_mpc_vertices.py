...
class centroidal_mpc:
  def __init__(self, initial, footstep_planner, params, CoM_ref, contact_trj_l, contact_trj_r):
    ...
    for i in range(self.N):
        cost += 3000*cs.sumsqr(self.opti_hw[:,i+1]) + \
              600*cs.sumsqr(self.opti_CoM[2,i]-0.72)  +\
              500*cs.sumsqr(self.opti_CoM[0,i+1]-self.opti_com_ref[0,i]) + \
              500*cs.sumsqr(self.opti_CoM[1,i+1]-self.opti_com_ref[1,i]) + \
              ...
              500* self.opti_hw[:,i+1].T @ self.opti_hw[:,i+1] - self.opti_hw[:,i].T @ self.opti_hw[:,i]
