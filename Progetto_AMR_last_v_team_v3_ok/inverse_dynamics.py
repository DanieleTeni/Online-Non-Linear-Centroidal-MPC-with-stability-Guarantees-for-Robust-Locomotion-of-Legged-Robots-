...
class InverseDynamics:
    def __init__(self, robot, redundant_dofs, track_hw=False, foot_size=0.1, µ=0.5):
        self.robot = robot
        self.dofs = self.robot.getNumDofs()  # degree of freedom, not number of joints
        self.d = foot_size / 2.
        self.µ = µ
        self.track_hw = track_hw  # if True, track the vertical component of angular momentum (hw)
    ...
    def get_joint_torques(self, desired, current, contact):
        ...
        pos_gains = {'lfoot': 15., 'rfoot': 15., 'com':  10., 'torso': 15., 'base': 15., 'joints': 20.}
        vel_gains = {'lfoot': 10., 'rfoot': 10., 'com': 10., 'torso': 2., 'base': 2., 'joints': 1.e-1}

        # If hw tracking is enabled, add an extra task "hw"
        if self.track_hw:
            tasks.append('hw')
            weights['hw']   = 1   # adjust this weight as needed
            pos_gains['hw'] = 1
            vel_gains['hw'] = 0
        ...
        # If hw tracking is enabled, set up the jacobian for hw using the torso's vertical row.
        if self.track_hw:
            torso_jac = self.robot.getAngularJacobian(torso, inCoordinatesOf=dart.dynamics.Frame.World())
            J['hw'] = torso_jac[2:3, :]  # vertical component (row 2), shape (1, dofs)
            Jdot['hw'] = np.zeros((1, self.dofs))  # assume zero derivative
        ...
        # Since we have no feedforward for hw, set it to zero if tracking is enabled.
        if self.track_hw:
            ff['hw'] = np.zeros(1)
        ...
        # If hw tracking is enabled, use the measured values (only z-component) from both desired and current.
        if self.track_hw:
            pos_error_hw = np.array([desired['hw']['val'][2]]) - np.array([current['hw']['val'][2]])
            vel_error_hw = np.array([0.0])  # no derivative available, set to zero
            pos_error['hw'] = pos_error_hw
            vel_error['hw'] = vel_error_hw
        ...
        A_ineq_template = np.array([[ 1, 0, 0, 0, 0, -self.d],
                                    [-1, 0, 0, 0, 0, -self.d],
                                    [0,  1, 0, 0, 0, -self.d],
                                    [0, -1, 0, 0, 0, -self.d],
                                    [0, 0, 0,  1, 0, -self.µ],
                                    ...
                                    [0, 0, 0, 0,  1, -self.µ],
                                    [0, 0, 0, 0, -1, -self.µ]])
        A_ineq[0:self.n_ineq_constraints, f_c_indices] = block_diag(A_ineq_template, A_ineq_template)
