...
// Include necessary headers from the BipedalLocomotionFramework for the MPC Controller
                        ...
                        std::ostream& centroidalMPCData)
{
    ...
    for (const auto& [key, contact] : mpc.getOutput().contacts)
    {
        ...
        if (contactList != contactLists.end())
        {
...
void runSimulation(bool enableStabilityConstraint, const std::string& outputFilename)
{
    ...
    handler->setParameter("enable_stability_cstr", enableStabilityConstraint);
    ...
    if (!mpc.initialize(handler))
    {
        throw std::runtime_error("MPC initialization failed.");
    }
    ...
    for (int i = 0; i < simulationHorizon; i++)
    {
        ...
        if (controllerIndex == 0)
        {
            ...
            mpc.setState(com, dcom, angularMomentum);
        ...
        if (i == 0)
        {
            writeHeaderOfFile(mpc.getOutput().contacts, file);
        }
    ...
    if (!com(0) > 0.25)
    {
        std::cerr << "The simulation did not reach the desired position." << std::endl;
    }

    if (!(std::abs(com(1)) < 0.1))
    {
        std::cerr << "The simulation did not reach the desired position." << std::endl;
    }

    if (!(std::abs(com(2) - 0.53) < 0.01))
    {
        std::cerr << "The simulation did not reach the desired position." << std::endl;
    }
...
int main()
{
    ...
    // runSimulation(stabilityActive, outputFilename)
    runSimulation(false, "centroidalMPCDataWithoutStabilityCstr.txt");
    runSimulation(true, "centroidalMPCDataWithStabilityCstr.txt");
    return 0;
}
