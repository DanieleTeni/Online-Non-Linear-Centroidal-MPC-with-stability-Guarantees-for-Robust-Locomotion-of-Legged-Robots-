...
class centroidal_mpc:
  def __init__(self, initial, footstep_planner, params, CoM_ref, contact_trj_l, contact_trj_r):
    ...
    self.next_des_pose_swing_MPC_at_update_time=np.zeros(6) # o un array di zeri
    self.next_des_pose_swing_MPC_at_69=np.zeros(6)
    ...
    #self.opti_pose_contact_l_trj = self.opt.parameter(6,self.N)
    #self.opti_pose_contact_r_trj = self.opt.parameter(6,self.N)
    #GAMMA_L
    ...
    for i in range(self.N):
      ...
      for j in range(3):  # x, y, z
          self.opt.subject_to((self.opti_pos_contact_l[j, i] - self.opti_pos_contact_trj_l_ref[j, i])*self.opti_contact_left[i] <= 0.025)
          self.opt.subject_to((self.opti_pos_contact_l[j, i] - self.opti_pos_contact_trj_l_ref[j, i])*self.opti_contact_left[i] >= -0.025)

          self.opt.subject_to((self.opti_pos_contact_r[j, i] - self.opti_pos_contact_trj_r_ref[j, i])*self.opti_contact_right[i] <= 0.025)
          self.opt.subject_to((self.opti_pos_contact_r[j, i] - self.opti_pos_contact_trj_r_ref[j, i])*self.opti_contact_right[i] >= -0.025)
    ...
    for i in range(self.N):
      self.aux_forces_average_l_mat[:,i]=(1/num_vertices_for_a_foot)*self.Vl_mat[:,i]*self.mass*self.opti_contact_left[i]
      self.aux_forces_average_r_mat[:,i]=(1/num_vertices_for_a_foot)*self.Vr_mat[:,i]*self.mass*self.opti_contact_right[i]
    ...
    for i in range(self.N):
      cost += 100*cs.sumsqr(self.opti_hw[:,i+1]) + \
              15000*cs.sumsqr(self.opti_CoM[0,i+1]-self.opti_com_ref[0,i]) + \
              15000*cs.sumsqr(self.opti_CoM[1,i+1]-self.opti_com_ref[1,i]) + \
              15000*cs.sumsqr(self.opti_CoM[2,i+1]-self.opti_com_ref[2,i]) + \
              8000*cs.sumsqr(self.opti_CoM[2,i+1]-0.72) + \
              20000*cs.sumsqr((self.opti_pos_contact_l[:,i]-self.opti_pos_contact_l_ref[:,i])*self.opti_contact_left[i]) + \
              20000*cs.sumsqr((self.opti_pos_contact_r[:,i]-self.opti_pos_contact_r_ref[:,i])*self.opti_contact_right[i]) + \
              20000*cs.sumsqr((self.opti_ang_contact_l[:,i]-self.opti_ang_contact_l_ref[:,i])*self.opti_contact_left[i]) + \
              20000*cs.sumsqr((self.opti_ang_contact_r[:,i]-self.opti_ang_contact_r_ref[:,i])*self.opti_contact_right[i]) + \
              ...
              10000* self.opti_hw[:,i+1].T @ self.opti_hw[:,i+1] - self.opti_hw[:,i].T @ self.opti_hw[:,i]

    '''
            weight = (i + 1) / self.N  # cresce da 1/N a 1
            cost += 400*cs.sumsqr(self.opti_hw[:,i+1]) + \
                  100*cs.sumsqr(self.opti_CoM[0,i]-self.opti_com_ref[0,i]) + \
                  100*cs.sumsqr(self.opti_CoM[1,i]-self.opti_com_ref[1,i]) + \
                  200*cs.sumsqr(self.opti_CoM[2,i]-self.opti_com_ref[2,i]) + \
                  100*cs.sumsqr((self.opti_pos_contact_l[:,i]-self.opti_pos_contact_l_ref[:,i])*self.opti_contact_left[i]) + \
                  200*cs.sumsqr((self.opti_pos_contact_r[:,i]-self.opti_pos_contact_r_ref[:,i])*self.opti_contact_right[i]) + \
                  200*cs.sumsqr((self.opti_ang_contact_l[:,i]-self.opti_ang_contact_l_ref[:,i])*self.opti_contact_left[i]) + \
                  200*cs.sumsqr((self.opti_ang_contact_r[:,i]-self.opti_ang_contact_r_ref[:,i])*self.opti_contact_right[i]) + \
                  cs.sumsqr(self.aux_forces_average_l_mat[:,i]-self.opti_v1l_force[:,i])*self.opti_contact_left[i] + \
                  cs.sumsqr(self.aux_forces_average_l_mat[:,i]-self.opti_v2l_force[:,i])*self.opti_contact_left[i] + \
                  cs.sumsqr(self.aux_forces_average_l_mat[:,i]-self.opti_v3l_force[:,i])*self.opti_contact_left[i] + \
                  cs.sumsqr(self.aux_forces_average_l_mat[:,i]-self.opti_v4l_force[:,i])*self.opti_contact_left[i] + \
                  cs.sumsqr(self.aux_forces_average_r_mat[:,i]-self.opti_v1r_force[:,i])*self.opti_contact_right[i] + \
                  cs.sumsqr(self.aux_forces_average_r_mat[:,i]-self.opti_v2r_force[:,i])*self.opti_contact_right[i] + \
                  cs.sumsqr(self.aux_forces_average_r_mat[:,i]-self.opti_v3r_force[:,i])*self.opti_contact_right[i] + \
                  cs.sumsqr(self.aux_forces_average_r_mat[:,i]-self.opti_v4r_force[:,i])*self.opti_contact_right[i] 
    '''
    ...
    self.model_state = {'com': {'pos': np.zeros(3), 'vel': np.zeros(3), 'acc': np.zeros(3)},
          ...
          'next_des_pose_swing_MPC_at_update_time' :{'val':np.zeros(6)},
          'next_des_pose_swing_MPC_at_69' :{'val':np.zeros(6)},
...
# let the mpc compute the state in the rest of the horizon
  ...
  def solve(self, current, t):
   ...
   # UPDATE  the contact status L/R over N
    ...
    #An: Update CoM_ref value for every step t and in an entire horizon N
    idx=0
    ...
    #  print(f"self.contact_trj_r[t+i][0]['pos']:{self.contact_trj_r[t+i][0]['pos']}")
  #UPDATING COMPLETED
  ...
  #Some print
    ...
    self.next_des_pose_swing_MPC_at_update_time=np.zeros(6)
    ...
    if self.params['update_contact']=='YES' :
      ...
      if t%self.update_time==0:
        ...
        if t!=0 :
          ...
          contact_phase_next_after_the_horizon= self.footstep_planner.get_phase_at_time(t+self.N-self.correction)
          if t>199:
            ...
            #Ritrieve next MPC optimal contact position for the swing foot
              ...
              if current_contact_foot=='lfoot':
                ...
                self.next_des_pose_swing_MPC_at_update_time = self.x_collect[18:24,self.N-self.correction]#mean the swing foot is the right one-> update the contact position of the swing foot
                ...
                self.next_des_pose_swing_MPC_at_update_time = self.x_collect[12:18,self.N-self.correction]
                print(f'to :{self.x_collect[12:18,self.N-self.correction]}')

      if t%((self.update_time-100)-self.correction+self.N-1)==0:
        if t!=0 :
          if t>199:
            if self.model_state['swing_foot']['rfoot']:
              self.next_des_pose_swing_MPC_at_69 = self.x_collect[18:24,0]#mean the swing foot is the right one-> update the contact position of the swing foot
            if self.model_state['swing_foot']['lfoot']:      
              self.next_des_pose_swing_MPC_at_69 = self.x_collect[12:18,0]#mean the swing foot is the right one-> update the contact position of the swing foot
            print(f't:{t} , self.next_des_pose_swing_MPC_at_69 :{self.next_des_pose_swing_MPC_at_69 }')    
    
    print(f'next_des_pose_swing_MPC_at_update_time:{self.next_des_pose_swing_MPC_at_update_time}')
    print(f't:{t} , self.next_des_pose_swing_MPC_at_69 :{self.next_des_pose_swing_MPC_at_69 }')    
    
    self.model_state['next_des_pose_swing_MPC_at_update_time']['val'] = np.array(self.next_des_pose_swing_MPC_at_update_time)
